    ...
    <style>
        ...
        *{margin:0;padding:0;box-sizing:border-box;font-family:'Poppins',sans-serif;}
        ...
        .welcome-banner{
            ...
            color:#fff;padding:30px;border-radius:8px;margin-bottom:30px;
            box-shadow:var(--shadow-sm);display:flex;align-items:center;gap:20px;
        }
        .welcome-banner h2{font-size:1.5rem;margin:0;}
        .welcome-banner p{margin:0;opacity:.9;}
        .menu-toggle{display:none;font-size:1.5rem;cursor:pointer;align-self:center;margin-right:auto;}
        .form-card,.jadwal-list{background:#fff;border-radius:8px;padding:25px;box-shadow:var(--shadow-sm);margin-bottom:30px;}
        .form-row{display:flex;gap:20px;margin-bottom:15px;}
        .form-group{flex:1;display:flex;flex-direction:column;gap:8px;}
        label{font-weight:500;font-size:.9rem;color:var(--text-secondary);}
        input[type="text"],input[type="date"],select{
            width:100%;padding:12px 15px;border:1px solid #e2e8f0;border-radius:6px;
            font-size:.95rem;transition:all .3s;background:#fff;height:48px;
        }
        input:focus,select:focus{border-color:var(--primary-color);box-shadow:0 0 0 3px rgba(30,58,138,.1);outline:none;}
        .btn{
            background:var(--primary-color);color:#fff;border:none;padding:12px 25px;border-radius:6px;
        ...
        .actions .btn{padding:6px 8px;border-radius:50%;width:34px;height:34px;justify-content:center;}
        .actions .btn i{margin:0;}
        @media(max-width:768px){
            .main-content{margin-left:0;padding:15px;max-width:100%;}
            .menu-toggle{display:block;}
            .welcome-banner{flex-direction:row;align-items:center;gap:15px;padding:20px;}
            .welcome-banner h2{font-size:1.2rem;margin:0;}
            .welcome-banner p{font-size:.9rem;margin:0;opacity:.9;}
...
<div class="main-content" id="mainContent">
    ...
    <div class="form-card">
        <form method="GET" action="">
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" id="search" name="search"
                           placeholder="Nama, Username, No. Rekening"
                        ...
                        if ($result_jurusan && $result_jurusan->num_rows > 0) {
                            ...
                            while ($row = $result_jurusan->fetch_assoc()) { ?>
                                <option value="<?php echo $row['id']; ?>"
                                        <?php echo $jurusan_filter == $row['id'] ? 'selected' : ''; ?>>
                    ...
                    <select id="tingkatan" name="tingkatan" onchange="updateKelasOptions()">
                        ...
                        if ($result_tingkatan && $result_tingkatan->num_rows > 0) {
                            ...
                            while ($row = $result_tingkatan->fetch_assoc()) { ?>
                                <option value="<?php echo $row['id']; ?>"
                                        <?php echo $tingkatan_filter == $row['id'] ? 'selected' : ''; ?>>
                    ...
                    <select id="kelas" name="kelas">
                        ...
                        if ($result_kelas && $result_kelas->num_rows > 0) {
                            ...
                            while ($row = $result_kelas->fetch_assoc()) { ?>
                                <option value="<?php echo $row['id']; ?>"
                                        <?php echo $kelas_filter == $row['id'] ? 'selected' : ''; ?>>
            ...
            <div style="display:flex;gap:10px;margin-top:10px;">
                ...
                <button type="button" id="resetBtn" class="btn btn-cancel">
    ...
    <div class="jadwal-list">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;">
            <h3><i class="fas fa-table"></i> Daftar Akun Siswa</h3>
        </div>
                        ...
                        <tr>
                            ...
                            <td style="font-weight:500;">
                                <?php if ($rekening['is_frozen']): ?>
                                    <i class="fas fa-lock" style="color:#dc2626; margin-right:5px;"></i>
                                <?php else: ?>
                                    <i class="fas fa-lock-open" style="color:#059669; margin-right:5px;"></i>
                                <?php endif; ?>
        ...
        <?php if ($total_pages > 1): ?>
            <div style="margin-top:20px;display:flex;justify-content:center;gap:5px;">
                <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                    <a href="?page=<?= $i ?>"
                       class="btn"
                       style="padding:8px 12px; <?= $i == $page ? 'background:var(--primary-dark);' : 'opacity:0.7;' ?>">
                        <?= $i ?>
                    </a>
                <?php endfor; ?>
            </div>
