...
while ($row = $result->fetch_assoc()) {
    // Gabungkan nama tingkatan dan kelas
    $kelas_parts = [];
...
class MYPDF extends TCPDF {
    ...
    public function Header() {
        if ($this->isFirstPage) {
            ...
            $this->Cell($col_width[4], 6, 'Oleh', 1, 0, 'C', true);
...
$pdf->Cell($colWidth, 6, 'Rp ' . number_format($totals['total_net'], 0, ',', '.'), 1, 1, 'R');
...
$pdf->Cell($col_width[2], 6, 'Nama Siswa', 1, 0, 'C', true);
$pdf->Cell($col_width[3], 6, 'Kelas', 1, 0, 'C', true);
$pdf->Cell($col_width[4], 6, 'Oleh', 1, 0, 'C', true);
...
if (!empty($transactions)) {
    ...
    foreach ($transactions as $row) {
        $jenis = ($row['jenis_transaksi'] === 'setor') ? 'Setor' : (($row['jenis_transaksi'] === 'tarik') ? 'Tarik' : ucfirst($row['jenis_transaksi']));
        if ($row['jenis_transaksi'] === 'setor') $total_setor += $row['jumlah'];
        if ($row['jenis_transaksi'] === 'tarik') $total_tarik += $row['jumlah'];
        ...
        $pdf->Cell($col_width[4], $row_height, htmlspecialchars($row['nama_oleh']), 1, 0, 'L');
        $pdf->Cell($col_width[5], $row_height, $jenis, 1, 0, 'C');
