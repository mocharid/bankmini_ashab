<?php
/**
 * Kelola Tingkatan Kelas - Final Version (Tambah Konfirmasi + Hapus Tanpa Bug)
 * File: pages/admin/kelola_tingkatan.php
 */
...
// Total tingkatan
$total_res = $conn->query("SELECT COUNT(*) as total FROM tingkatan_kelas");
$total_rows = $total_res->fetch_assoc()['total'];
$total_pages = ceil($total_rows / $items_per_page);

// ======================================== HANDLE POST ========================================
// Hanya proses dari form langsung (tambah/edit) dan hapus dari SweetAlert
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['token']) && $_POST['token'] === $token) {
    ...
    if (isset($_POST['hapus_id'])) {
        ...
        if ($cek > 0) {
            setFlash('error', 'Tidak dapat menghapus tingkatan yang masih digunakan di kelas!');
        } else {
            $conn->query("DELETE FROM tingkatan_kelas WHERE id = $id");
            setFlash('success', 'Tingkatan berhasil dihapus!');
        }
        $_SESSION['form_token'] = bin2hex(random_bytes(32));
        $token = $_SESSION['form_token'];
    ...
    // TAMBAH / EDIT (dari form langsung)
    $nama = trim($_POST['nama_tingkatan']);

    if (empty($nama)) {
        setFlash('error', 'Nama tingkatan tidak boleh kosong!');
    } else {
        if (isset($_POST['edit_id']) && !empty($_POST['edit_id'])) {
            ...
            $old->bind_param("i", $id);
            $old->execute();
            $old_name = $old->get_result()->fetch_assoc()['nama_tingkatan'] ?? '';

            if ($nama === $old_name) {
                setFlash('info', 'Tidak ada perubahan yang disimpan.');
            } else {
                $cek = $conn->prepare("SELECT id FROM tingkatan_kelas WHERE nama_tingkatan = ? AND id != ?");
                $cek->bind_param("si", $nama, $id);
                $cek->execute();
                if ($cek->get_result()->num_rows > 0) {
                    setFlash('error', 'Nama tingkatan sudah ada!');
                } else {
                    $upd = $conn->prepare("UPDATE tingkatan_kelas SET nama_tingkatan = ? WHERE id = ?");
                    $upd->bind_param("si", $nama, $id);
                    $upd->execute();
                    setFlash('success', 'Tingkatan berhasil diperbarui!');
                }
            }
        } else {
            // TAMBAH BARU
            $cek = $conn->prepare("SELECT id FROM tingkatan_kelas WHERE nama_tingkatan = ?");
            $cek->bind_param("s", $nama);
            $cek->execute();
            if ($cek->get_result()->num_rows > 0) {
                setFlash('error', 'Tingkatan sudah ada!');
            } else {
                $ins = $conn->prepare("INSERT INTO tingkatan_kelas (nama_tingkatan) VALUES (?)");
                $ins->bind_param("s", $nama);
                $ins->execute();
                setFlash('success', 'Tingkatan berhasil ditambahkan!');
            }
        }
    }
...
if (isset($_GET['edit']) && is_numeric($_GET['edit'])) {
    ...
    if ($row) {
        ...
        setFlash('error', 'Tingkatan tidak ditemukan!');
...
$stmt = $conn->prepare("SELECT id, nama_tingkatan FROM tingkatan_kelas ORDER BY nama_tingkatan ASC LIMIT ? OFFSET ?");
$stmt->bind_param("ii", $items_per_page, $offset);
$stmt->execute();
$res = $stmt->get_result();
$tingkatan_list = $res->fetch_all(MYSQLI_ASSOC);
$stmt->close();
?>
...
<body>
                        ...
                        <?php if (empty($tingkatan_list)): ?>
                            <tr>
                                <td colspan="3" style="text-align:center; padding:40px; color:#999;">
                                    Belum ada data tingkatan kelas
                                </td>
                            </tr>
                        <?php else: ?>
                            <?php foreach ($tingkatan_list as $i => $t): ?>
                                <tr>
                                    <td><?= $offset + $i + 1 ?></td>
    ...
    <script>
        ...
        document.getElementById('submitBtn').addEventListener('click', function(e) {
            ...
            if (!nama) {
                Swal.fire('Gagal', 'Nama tingkatan tidak boleh kosong!', 'error');
                return;
            }

            Swal.fire({
                title: <?= $edit_id ? "'Update Tingkatan?'" : "'Tambah Tingkatan?'" ?>,
        ...
        // Hapus Tingkatan
        function hapusTingkatan(id, nama) {
            Swal.fire({
