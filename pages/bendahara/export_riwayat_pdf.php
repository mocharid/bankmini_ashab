 ...
 * File: pages/bendahara/export_riwayat_pdf.php
 */
...
if (!empty($search_query)) {
    $where_clauses[] = "(u.nama LIKE ? OR ptr.no_pembayaran LIKE ? OR pi.nama_item LIKE ?)";
...
// Main Query (tanpa limit untuk export semua)
$sql_data = "SELECT 
                ptr.id, 
                ptr.no_pembayaran, 
                ptr.tanggal_bayar, 
...
function formatTanggalSingkat($date)
{
    ...
    return $day . ' ' . $bulan[$month] . ' ' . $year . ' ' . $time;
}
...
// CREATE PDF
// ============================================
$pdf = new MYPDF('L', 'mm', 'A4', true, 'UTF-8', false); // Landscape for more columns
$pdf->SetCreator('Schobank SMK Plus Ashabulyamin');
$pdf->SetAuthor('Bendahara');
$pdf->SetTitle('Laporan Riwayat Pembayaran');
...
$pdf->Cell(0, 8, 'LAPORAN RIWAYAT PEMBAYARAN', 0, 1, 'C');

// Filter Info
$pdf->SetFont('helvetica', '', 10);
$pdf->SetTextColor(90, 90, 90);
$periode_text = formatTanggalIndo($date_start) . ' - ' . formatTanggalIndo($date_end);
$pdf->Cell(0, 6, 'Periode: ' . $periode_text, 0, 1, 'C');
$pdf->Cell(0, 6, 'Pembayaran Tagihan: ' . $item_name, 0, 1, 'C');
if (!empty($search_query)) {
    $pdf->Cell(0, 6, 'Kata Kunci: ' . $search_query, 0, 1, 'C');
}
$pdf->Ln(8);
...
$w = [12, 45, 50, 50, 45, 50, 21];
$headers = ['No', 'Tanggal Pembayaran', 'No Transaksi', 'Nama', 'Kelas', 'Jurusan', 'Status'];
...
foreach ($data as $row) {
    ...
    $pdf->Cell($w[1], 7, formatTanggalSingkat($row['tanggal_bayar']), 1, 0, 'C', $fill);
    ...
    // Status
    $pdf->Cell($w[6], 7, 'Lunas', 1, 1, 'C', $fill);
...
// === OUTPUT PDF ===
$filename = 'Riwayat_Pembayaran_' . date('Y-m-d_His') . '.pdf';
